...
public abstract class Pokemon implements Comparable<Pokemon> {
   ...
   **/
   public Pokemon(String species, String name, 
      ...
      if (this.name.length() == 0) {
         PokemonException pe = new PokemonException("The name cannot be empty");
         throw pe;
      }
  ...
  */
   ...
   * @param victim compare this Pokemon.
   * @return int of the compare result
   */
   public int compareTo(Pokemon victim) {
      int result = 0; //result
      
      //if equal next
      result = this.getNumber() - victim.getNumber();
      if (result == 0) { //if equal next
         result = this.getName().compareTo(victim.getName());
      } 
      if (result == 0) { //if equal next
         result = this.getHP() - victim.getHP();
      }
      if (result == 0) { //if equal next
         result = this.getCP() - victim.getCP();
      }
      if (result == 0) { //if equal next
         result = this.getFastAttack().compareTo(victim.getFastAttack());
      }
      if (result == 0) { //if equal next
         result = this.getSpecialAttack().compareTo(victim.getSpecialAttack());
      }
   ...
   * Compare two Pokemons.
   * @param victim compare this pokemon.
   * @return boolean of the compare result
   */
   public boolean equals(Pokemon victim) {
      int result = 0; //result
